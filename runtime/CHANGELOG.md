...
- **Deactivate button in Dist Manager** + **`wb runtime deactivate` CLI** — yad `--list` button labels are static (gtk doesn't fire callbacks on row selection without a click), so a single button can't morph between "Activate" and "Deactivate" based on which row is highlighted. Instead added a separate Deactivate button alongside Activate; both handlers are context-aware (`Activate` on an already-active row → "Already active" info; `Deactivate` on a non-active row → "Not active" info). New `wb_dist_clear_alias` helper (`wb-lib/wb-dist.sh`) atomically removes the WINE-BLEEDING symlink, exposed via `wb runtime deactivate`. Provides a way to return to "no dist active" state without removing the dist.
