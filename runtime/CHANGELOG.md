...
- **Default GPU dropdown lists actual hardware** instead of the generic `Auto / NVIDIA / AMD / Intel` placeholders. New `_wb_gui_detect_gpus` parses `lspci -nn` output for PCI base class 0x03 (Display Controller — covers VGA 0x0300, 3D 0x0302, Other Display 0x0380), pulls the bracketed product name (e.g. `GeForce RTX 4080 SUPER`, `Radeon RX 7900 XTX`, `Raptor Lake-S UHD Graphics`), maps each entry back to its vendor enum (`nvidia`/`amd`/`intel`) on save, and falls back to the generic vendor list on hosts without `lspci` or with no display class device. Storage layer unchanged (`wb_gui_settings_set_general gpu` still writes vendor enum), so existing config + bats tests stay valid.
- **Install flow no longer silently closes after the Install button** — `_cmd_install_flow` ran `"${_WB_BIN}" run "${installer_exe}" --prefix "${prefix_name}" --wait 2>/dev/null || true`, swallowing wb's stderr. When the user picked "Create new prefix..." in the prefix-select step, the prefix never got initialised, and `wb run` aborted with `prefix '<name>' does not exist (path: ...). run 'wb prefix init <name>' first` — but stderr was discarded, the dialog quietly closed, and the user saw no feedback. Fix: (1) immediately after the prefix-select step (and BEFORE the "Ready to install" confirm dialog), auto-`wb prefix init` the prefix if its `drive_c/` is missing on disk — surfacing any init failure in an error dialog with the actual exit code + stderr at the earliest natural point in the flow, so the user never sees a confirm prompt for an installer that can never start. (2) `wb run`'s stderr is captured too — if wb itself rejected the call (heuristic: stderr line starts with `wb run:` meaning wb refused before exec, e.g. dist/lock issue), pop an "Install could not start" dialog with the real reason; plain installer-exit non-zero (lots of installers exit 1 even on success) keeps proceeding to detection.
- **Per-card GPU pinning at app launch** — picking a specific card from the Default GPU dropdown now actually steers DXVK and VKD3D-Proton to that Vulkan device instead of just storing the vendor enum. The dialog persists the literal label (e.g. `GeForce RTX 4080 SUPER`) to `general.json`'s new `gpu_label` field; `wb_env_compose` reads it on every launch and emits `DXVK_FILTER_DEVICE_NAME=<label>` + `VKD3D_FILTER_DEVICE_NAME=<label>` (both runtimes do a case-insensitive substring match against the Vulkan device name reported by `VK_EXT_physical_device_properties2`). Resolves multi-card ambiguity that mattered on the user's box (RTX 5080 + RTX 4080 SUPER — same vendor, different physical cards). Auto / generic-vendor selections clear the override, restoring driver-default device pick. `WB_GPU_LABEL` env var takes precedence over the JSON for per-app/per-prefix overrides via wb-config layering. New `10_env.bats` tests 28-30 cover the WB_GPU_LABEL → filter env path, the JSON → filter env path, and the empty-label → no-emit path.
- **`_wb_gui_validate_exe_path` accepts Unicode paths** (Cyrillic / CJK / accented Latin / emoji). Previous regex was an ASCII-only whitelist `^/[-A-Za-z0-9_./ ]+$`, which rejected real user paths like `~/Загрузки/installer.exe` (Russian *Downloads*), `~/桌面/setup.exe` (Chinese *Desktop*), `~/Téléchargements/x.exe` (French *Downloads*) with the misleading `Invalid installer: The path '<...>' is not a valid executable path.` dialog. Replaced with a control-char + quote-char blacklist; shell-meta safety check (`;|&` ` `$()<>{}`) and path-traversal `..` check are unchanged, so the security profile is the same. New `24_gui.bats` test 12 covers four accept cases (Cyrillic, CJK, accented Latin, ASCII with spaces) + four reject cases (`..`, `$evil`, `;`, relative path).
- **`_wb_validate_path_arg` (wb runtime) accepts Unicode paths** — companion fix to the wb-gui validator above. Same ASCII-only whitelist `^/[A-Za-z0-9_./-]+$` lived in `wb`'s top-level path-arg validator (used by `wb run`, `wb prefix init`, etc.) and rejected the same set of real user paths after the install flow had already initialised the prefix and was about to hand off to `wb run "/home/palgin/Загрузки/setup.exe"` — the user saw `wb run: invalid PATH '/home/palgin/Загрузки/setup.exe' (expected [A-Za-z0-9_./-]+)` after the prefix-creation step succeeded, exactly the same regression as the GUI-side one but on the next code path. Replaced with the same defence-in-depth model: reject `..` (traversal), reject the shell-meta set `;|&` ` `$()<>{}`, reject control characters / `"` / `'` / `\`, but accept any other UTF-8 codepoint inside an absolute path. Relative names (single-segment slugs like prefix or dist identifiers) stay ASCII-strict because they are project-internal slugs, not filesystem paths. New `00_sanity.bats` test 4 covers four accept cases (Cyrillic, CJK, accented Latin, ASCII-with-spaces, plus a relative-name accept) + four reject cases (`..`, `$evil`, `;`, non-ASCII relative name); the test wraps each call in a `( ... )` subshell because the validator uses `exit 1` on rejection (correct for top-level wb invocation) and inline `if`-condition calls would otherwise tear down the whole test shell.
- **Per-app Settings is now a single-window dialog** (`_cmd_app_prefs`) — same root cause and same fix as the General Prefs migration. Pressing Settings on a selected app row used to call `_cmd_settings_v2 app <id>`, which spawns 4 `yad --plug` children (General / Dist / Prefix / Per-App tabs) and re-attaches them to a `yad --notebook` parent. On Plasma 6 / Wayland with XWayland (the user's setup) the EmbedProto handshake doesn't complete cleanly, so the four panes detach and render as four separate top-level windows with mismatched titles, no main button bar, and the wrong icon — exactly the "multiple non-wired windows, wrong icons, incorrect formatting" symptom the user reported, identical to the Prefs disaster fixed earlier. The replacement is a single `wb_gui_yad --form` containing all the per-app fields the Tab 4 plug used to render — Windows version (CB) + inherited-value RO label, WINEDEBUG (TEXT) + inherited RO, Extra Wine args (TEXT), Env vars (TXT multi-line), and the full Overlays panel (MangoHud / VKBasalt / OptiScaler CHK + source/version CB + action BTN + MangoHud config FL + Check-for-updates BTN + last-checked LBL). Field declaration order is identical to the plug version's so the existing `_settings_save_app` parser is reused unchanged: form output is written to a temp file and handed off, keeping save semantics (winver inherit-clear via `del(.win_version)`, env-var KEY=VALUE parsing, overlay-install auto-prompt, env-bake) byte-for-byte identical between the legacy 4-tab path and the new single-window path. The legacy notebook stays callable as `wb-gui settings-v2 app <id>` for tooling/tests; only the GUI app-list rc=30 button now routes to the single-window dialog.

### Apps menu polish (window size, selection, app names, Remove wording, overlay buttons)

- **Apps list opens at a usable size** — added `--width=860 --height=480` to the apps `--list`. yad's default geometry was tiny (cropped the LAST PLAYED column and forced single-row scrolling), and there was no explicit override on the apps menu while every other dialog in wb-gui sets one. 860×480 fits four columns + button bar without truncation on a 1080p display and is still small enough not to overlap the desktop on tiny screens.
- **Selection actually selects** — removed `--print-all` from the apps `--list`. With `--print-all`, yad emits **every** row's columns to stdout joined by the row separator; the `IFS='|' read -r ... <<< "${yad_out}"` parser then only consumes the first record, so the Run / Settings / Remove buttons always operated on row 1 regardless of which row was highlighted (user-visible: "selecting an item from the table doesn't work, it always chooses the first row"). Without `--print-all`, yad emits only the highlighted row's columns — which is what every other `--list` consumer in wb-gui already expects. The post-install detection checklist (`_gui_post_install_prompt`) keeps `--print-all` because that one IS a checklist and needs the TRUE/FALSE state of every row.
- **NAME column shows a clean app name, not the literal Windows path** — the python lnk parser (`runtime/libexec/wb-lnk-parse.py`) used `os.path.basename(target_path)` to derive `display_name` when the .lnk had no embedded NAME StringData. On Linux, `os.path.basename` only splits on `/`, so for a Windows path like `C:\Program Files\TrackChecker\TrackChecker.exe` it returned the **whole string unchanged**, which then got persisted as the app's `.name` in `apps.json`. Replaced with a Windows-aware basename helper (`p.rsplit("\\", 1)[-1]` then `os.path.splitext`) so the fallback yields `TrackChecker`. For legacy entries already written with the full path as name, `_gui_build_app_rows` now display-cleans on render (strips everything up to and including the last `\` and drops the `.exe` suffix), so existing apps.json entries don't need migration to look right in the GUI.
- **Remove dialog actually shows backslashes and explains what Remove does** — yad / glib's `--text` passes the string through escape processing (`g_strcompress`-style) which silently consumes unrecognised `\X` sequences; `Remove 'C:\Program Files\TrackChecker\TrackChecker'` rendered as `Remove 'C:Program FilesTrackCheckerTrackChecker'` (backslashes vanished, words run together). Display-escape backslashes via `${name//\\/\\\\}` so they survive yad's escape pass, and rewrote the dialog body to be explicit: "This only deregisters the app — it does NOT uninstall the program from the Wine prefix. The prefix '<name>' and all its installed files stay on disk. To actually uninstall the Windows program, run its uninstaller from inside the prefix..." with a concrete example. The success-info dialog likewise spells out that the prefix and installed files were not touched.
- **Uninstaller shortcuts (`unins*.exe`, `unwise.exe`, `uninst*.exe`, `setup*.exe`, `installer.exe`, `update*.exe`, `crashhandler*.exe`, `crashreport*.exe`, `_*.exe`) no longer auto-register** — the LNK detection path (Mechanism 1) had no blacklist; only the Program Files heuristic (`_wb_detect_pick_main_exe`) was filtering uninstallers. Inno Setup creates a Start Menu uninstall shortcut alongside the main shortcut, and the LNK loop happily registered it as a separate "app". Added the same blacklist (extended to also cover NSIS's `unwise*.exe` and Wise/InstallShield's `uninst*.exe`) at the M1 candidate-emit step, so `unins000.exe` and friends are dropped before they reach the post-install candidate checklist.
- **Overlay action buttons (Install / Update / Remove / Check-for-updates) actually work** — the four `_wb_overlay_btn_*` handlers in the per-app Settings overlays panel were exported via `export -f`, but the helpers they call (`_overlay_single_update_flow`, `_overlay_install_selected_flow`, `_overlay_check_all_dialog`, `_overlay_action_label`, `_overlay_run_install_with_log`, `_overlay_build_version_cb`, `_overlay_last_checked_text`, plus `_wb_gui_home`) live in the main `wb-gui` script and were **not** exported. yad spawns BTN-field commands via `bash -c` (a fresh shell that does NOT inherit non-exported function definitions); the BTN subshell sourced `wb-gui-overlays.sh` as instructed, but the helpers it actually needed weren't there, so the call resolved to "command not found" and the surrounding `2>/dev/null || true` swallowed the error — buttons silently no-op'd. Added an `export -f` block right after the BTN handlers for every transitively-required helper so the fresh subshell can resolve them. Now `Update` actually checks the GitHub release feed, `Install` actually downloads and stages the bundled overlay, and `Check for updates` actually walks all three overlays and shows the install-checklist dialog.
- **Stale-entry sweep on apps menu open + Refresh** (`_gui_purge_missing_apps`) — when the user uninstalls a Windows program from inside the prefix (running its `unins000.exe` / `setup.exe /uninstall`, deleting the install dir, etc.), the wine-bleeding app-list entry pointing at the now-missing `.exe` would silently linger; clicking Run on it produced a Wine "file not found" error. Walks `apps.json` on apps-menu open and on Refresh; any registered app whose `.exe` path no longer exists on disk gets surfaced in a single batch checklist (`yad --list --checklist`, pre-checked) titled "stale entries", with the message "<n> apps in your list have executables that are no longer on disk — likely you uninstalled them from inside Wine, or the prefix was deleted. Pick which stale entries to drop from wine-bleeding's app list:". User clicks Drop selected → `wb_gui_apps_remove` is called for each ticked row; or Skip to leave them all alone. Silent no-op when every registered exe still exists, so the user only ever sees this dialog when there's actual cleanup to do. Same display-cleaning rule as the main list (strip up-to-last-`\` + drop `.exe`) so the checklist row matches what the user saw before uninstalling. Equivalent to PortProton's "app no longer available — remove from list?" prompt, but batched into one dialog instead of one-per-app to avoid prompt fatigue.
- **Overlay install no longer fails on a fresh install with `Invalid --dest '<wb_home>/overlays': does not exist or is not a directory`** — `wb_gui_overlays_install` (`wb-gui-lib/wb-gui-overlays.sh`) computes `overlay_root=$WB_HOME/overlays` + `cache_dir=$WB_HOME/overlays/.cache` and passes them to `tools/fetch-overlay.sh` via `--dest` / `--cache`, but it never created the directories first. fetch-overlay's `--dest` validator (`tools/fetch-overlay.sh:218`) refuses to proceed if the directory is missing (exit code 65), so the very first overlay install on a fresh wb-gui setup always errored out the install-log dialog. `mkdir -p` both directories before invoking the fetcher, with explicit error reporting if the create fails (rather than letting fetch-overlay surface a confusing path-validator error).
- **Per-app overlay panel correctly remembers `bundled: false` (System) on re-open** — the `_ov_field` helper used inside `_cmd_app_prefs` (and the legacy `_launch_tab_app`) read each per-overlay field via `jq -r '.mangohud.bundled // empty'` and bash-defaulted on empty: `${v:-${def}}`. jq's `//` operator treats **both `null` AND `false`** as the "use the alternative" branch, so a stored `{"bundled": false}` (the user picked **System** in the source CB and saved) jq-rendered to empty, the bash default `'true'` won out, and the dialog re-rendered showing **Bundled** selected. Effect: the user picked System → saved → reopened the dialog → saw Bundled again → reported "per-app settings are not persistent". Replaced the `// empty` shorthand with an explicit `if type == "null" then "" else (. | tostring) end` so genuine null/missing values still fall back to the default but stored `false` survives as the literal string `"false"`. Smoke-tested across `enabled=true`, `bundled=false`, `version=null`, `config_path=null`, and a missing key — only the actually-null cases trigger the bash default; `false` is preserved. Same fix applied to both `_ov_field` definitions (the `_cmd_app_prefs` single-window version and the legacy `_launch_tab_app` plug-tab version still callable via `wb-gui settings-v2 app <id>`).
- **Bundled MangoHud install works against MangoHud 0.7+ release format** — upstream changed their release tarball layout in the 0.7 line: instead of a flat tree containing `lib/mangohud/libMangoHud.so` directly, the outer `MangoHud-X.Y.Z.tar.gz` now contains `MangoHud/MangoHud-package.tar` (the actual file tree, prefixed `./usr/{bin,lib,share}/...`) plus a user-targeted `mangohud-setup.sh` installer that drops files into `~/.local/...` (we don't want that — we install to `$WB_HOME/overlays/mangohud/<ver>/`). Plain `tar xf outer.tar.gz -C staging/` produced just `staging/MangoHud/MangoHud-package.tar` + `staging/MangoHud/mangohud-setup.sh`, so the staging-sentinel check (`lib/mangohud/libMangoHud.so`) failed every time and the install errored out with `Staging failure: expected file missing`. Two-stage extract added to `tools/fetch-overlay.sh`'s `prebuilt-tarball` case (gated on `OVERLAY == mangohud` so other overlay kinds keep their existing path): outer tarball into a scratch dir under staging, locate the inner `MangoHud-package.tar` via `find -maxdepth 3 -name 'MangoHud-package.tar'`, then `tar xf inner.tar -C staging --strip-components=2` to drop the leading `./usr/` and land `bin/`, `lib/`, `share/` directly at staging root. New layout is `lib/mangohud/{lib32,lib64}/libMangoHud.so` (mirrors the typical distro `/usr/lib/mangohud/...` packaging), so the sentinel was also bumped from `lib/mangohud/libMangoHud.so` → `lib/mangohud/lib64/libMangoHud.so` in both `tools/fetch-overlay.sh:WB_OVERLAY_SENTINEL` and `wb-gui-lib/wb-gui-overlays.sh:_wb_gui_overlays_sentinel_file`. The `LD_LIBRARY_PATH` emission for bundled mode (both the legacy emit-path and the per-app env-bake) was likewise updated to include both `lib/mangohud/lib64` and `lib/mangohud/lib32` so 64-bit and 32-bit Wine binaries each find the right `.so`. Finally, the Vulkan implicit-layer manifests (`MangoHud.x86_64.json`, `MangoHud.x86.json`) hardcode an absolute `library_path` (`/usr/lib/mangohud/lib64/libMangoHud.so`) which is correct for distro installs but wrong for our bundled location — the Vulkan loader uses that absolute path verbatim, no relative resolution. Post-extraction `sed -i -E 's|("library_path"...:..."[^"]*"|...resolved bundled path...|'` rewrites both manifests to point at the actual `${INSTALL_DIR}/lib/mangohud/lib{32,64}/libMangoHud.so`. Verified end-to-end on a fresh setup: `bash tools/fetch-overlay.sh --overlay mangohud --version latest --dest ... --cache ...` resolves v0.8.3 from GitHub, fetches the 11 MB outer tarball (or hits cache), unpacks both layers, validates the sentinel, atomic-swaps into `$WB_HOME/overlays/mangohud/0.8.3/`, and the rewritten manifest points at `${WB_HOME}/overlays/mangohud/0.8.3/lib/mangohud/lib{32,64}/libMangoHud.so`. Bats tests `33_overlays_registry.bats` and `35_overlay_panel.bats` updated to plant the sentinel under the new `lib/mangohud/lib64/` path.
- **Per-app `env_vars` from `settings/apps/<id>.json` actually reach the wine launch** — the wb-gui per-app Settings dialog (and the overlay panel) wrote env-var overrides into `$WB_HOME/settings/apps/<app_id>.json`'s `.env_vars` map (`MANGOHUD=1` when MangoHud is enabled, `ENABLE_VKBASALT=1` for VKBasalt, custom user-set keys, the WINEDLLOVERRIDES tweak for OptiScaler, etc.), but **`wb run` never read that file** — `wb_env_compose` only emits `WB_*` env-var-driven defaults, with no codepath that opened the per-app settings file. So the user could tick "Enable MangoHud" in the GUI on a host where MangoHud was system-installed, the overlay save logic would correctly bake `{"MANGOHUD":"1"}` into the per-app settings file, but launching the app left `MANGOHUD` unset in Wine's environment and no overlay appeared. New `_wb_apps_env_vars_for_exe <exe>` helper in `wb` looks up the app entry in `apps.json` by exact `.exe` match, reads `settings/apps/<app_id>.json`, and emits `.env_vars` as KEY=VALUE lines; `cmd_run` and `cmd_exec` append the result to the env_pairs array AFTER `wb_env_compose`'s defaults so `env A=1 A=2 wine` semantics let the per-app value win (env(1) honours the LAST KEY=VALUE for any key). Silent no-op when apps.json is missing, no entry matches, the per-app file is missing, or `.env_vars` is null/empty — so unconfigured apps keep launching with the same env they always had.
- **Per-app Settings overlay panel actually probes for system-installed packages** (`wb_gui_overlays_system_installed`) — the source dropdown ("Bundled / System") was a hardcoded yad CB option list that never checked whether the user's system actually has MangoHud or VKBasalt installed; the user could pick "System" → save → launch and Wine would silently use the bundled path or fail at launch because nothing provided `libMangoHud.so` / vkBasalt. New helper in `wb-gui-overlays.sh` does real detection: MangoHud via `command -v mangohud` plus a probe for `libMangoHud.so` in the standard linker search paths (`/usr/lib`, `/usr/lib64`, multiarch dirs, `/usr/local/lib*`); VKBasalt via `libvkbasalt.so` library probe plus a Vulkan implicit-layer manifest probe (`/usr/share/vulkan/implicit_layer.d/vkBasalt.json` and the multiarch/`/etc` variants) which is the canonical install marker since vkbasalt has no CLI. OptiScaler always returns false (Windows DLL, no system equivalent). The per-app Settings panel calls the probe when building the overlay rows: when the system package IS detected the source CB still offers "Bundled!System" and the row label reads "MangoHud (FPS/frametime HUD) — system package detected"; when it ISN'T, the CB collapses to a single "Bundled" entry (yad permits a one-item CB) and the row keeps the bundled-fallback "[not installed]" annotation when nothing's installed. `wb_gui_overlays_install` is also pre-creating `$WB_HOME/overlays` + `.cache` directories now (fix above), so the Bundled-source Install button finally succeeds end-to-end on a fresh setup.
