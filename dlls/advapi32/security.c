...
BOOL ADVAPI_GetComputerSid(PSID sid)
{
    ...
    if (!initialized)
    {
        /* Derive domain SID from the current user's token SID to match
         * the domain SID generated by wineserver from /etc/machine-id.
         * This ensures LookupAccountSid correctly maps user SIDs. */
        HANDLE token;
        initialized = TRUE;
        if (OpenProcessToken( GetCurrentProcess(), TOKEN_QUERY, &token ))
        {
            ...
            {
                SID *user_sid = ((TOKEN_USER *)buf)->User.Sid;
                if (user_sid->SubAuthorityCount >= 4 &&
                    user_sid->SubAuthority[0] == SECURITY_NT_NON_UNIQUE)
                {
                    computer_sid.SubAuthority[1] = user_sid->SubAuthority[1];
                    computer_sid.SubAuthority[2] = user_sid->SubAuthority[2];
                    computer_sid.SubAuthority[3] = user_sid->SubAuthority[3];
                }
            }
            CloseHandle( token );
        }
